...
*   **`A.5` Геометрия фазовых портретов:** Топологический анализ и построение фазовых траекторий в окрестности орбитальных резонансов.
...
*   **`B.0`—`B.1`:** Краткий обзор методов и регулярный метод малого параметра Пуанкаре.
*   **`B.2`—`B.3`:** Метод Ван дер Поля, анализ малых возмущений в консервативных системах и релаксационные колебания в нелинейных осцилляторах.
*   **`B.4`:** Метод Крылова-Боголюбова-Митропольского (КБМ) и разделение движений на медленные и быстрые составляющие.
*   **`B.5`:** Метод фон Цейпеля на основе неявных производящих функций [Brouwer, 1959; 0.1.25].
*   **`B.6`:** Метод преобразований Ли. Реализация алгоритма явных операторных сдвигов Ли и рекуррентных таблиц усреднения [Deprit, 1969; 0.1.25].
...
*   **Черновик `C.4`:** Развертывание аналитической теории возмущений Каулы в индексах гармоник геопотенциала.
*   **`C.5`:** Математическая архитектура и алгоритм аналитической теории SGP4, применяемой в оперативной космической баллистике.

### Раздел D. Вычислительные научно-исследовательские черновики (DRAFT)
...
*   **Черновики `D.2`:** Исследование влияния основных пертурбационных факторов на элементы НОО и моделирование неавтономного давления солнечного света (SRP) в переменных Уиттекера [Whittaker, 1937; 0.1.28].
*   **Черновики `D.3`:** Моделирование диссипативной силы аэродинамического сопротивления верхней атмосферы Земли и построение усредненных уравнений пассивного движения спутника по неканоническому алгоритму Депри-Камела [Kamel, 1970; Брумберг, 1980; 0.1.28].
